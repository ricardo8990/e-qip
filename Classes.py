...
class JamesBond(pygame.sprite.Sprite):
    ...
    def update(self, down, left, right, camera, level):  #updates james' position according to key presses
        ...
        if right:  #if righ key is pressed
            self.image = james_right.convert_alpha()  #change bond image to skiing right
            self.x = game_james_x_step #set speed

        if self.get_position_relative()[0] + self.x < 0:  #out of bounds control for min x position
            self.set_position_relative(0)
        elif self.get_position_relative()[0] + self.x > self.level_width - self.image.get_width():  #out of bounds control for max x position
        ...
        if self.get_position_relative()[1] + self.y > self.level_height - self.image.get_height():  #out of bounds control for y position
    ...
    def collide(self, all_sprites_list):  #collision detection for james bond
        ...
        for obstacle in all_obstacles_collided:  #for all obstaces which have had collisions
            if isinstance(obstacle, Agent): #if they are agents
                agent_sound.play()
                if self.lives == 0: #the following determines the life/death/health attributes of james bond to loop the game
                    self.dead = True  
                else:
...
class Obstacle(pygame.sprite.Sprite):  #obstacle
    ...
    def update(self, camera):
        self.rect = camera.apply(self)  #apply the camera rectangle to the obstacle
    '''
        ...
        for i in range(number):  # maps the designated number of obstacles
            ...
            for j in range(number_row):  
                col += step  #step number is the distance between to objects.  This is 0 by default.  
                             #But 30 for coins, because they are placed consequtively and we don't want them to overlap.
                location = [row, col] 
                if not (location in obstacle_loc):  # makes sure two obstacles are not in the same location
                    obstacle_loc.append(location)  #holds the list of obstacle locations that have previously been placed
...
class Agent(Obstacle): #enemy agent
    ...
    def track_player(self, player, level): #tracking james bond
        try:
            ...
            dx, dy = float(dx / self.dist.real), float(dy / self.dist.real)  #determine the direction that agent is going to travel on
            self.rect.x -= dx * 3 #add some speed
            self.rect.y -= dy * 4 * level  #add some speed.  this speed increases each level. agents travel faster in y direction
        except:
            pass
...
#Helps follow james bond on the screen as he skiis down.
class Camera(object):  #camera
    ...
    def apply(self, target):
        return target.rect.move(self.rect_pos.topleft)  #target is moved by the given offset, which is the camera's topleft position

    def update(self, target):
        self.rect_pos = self.tracker_func(self.rect_pos, target.rect)  #changes the camera position, according to target's values
        #which will be james bond
