...
def show_points(screen, player):  # prints the player name,points,power on the screen
    ...
    print_text(screen, first_line_text, 0, 70, game_default_text_size, pygame.color.THECOLORS["black"])
    print_text(screen, second_line_text, 0, 80, game_default_text_size, pygame.color.THECOLORS["black"])
    print_text(screen, third_line_text, 0, 90, game_default_text_size, pygame.color.THECOLORS["black"])


def start_animation(james, screen, all_sprites_list, clock, bg_img, level):  # it shows the initial animation
    r = animation_letter_size
    while james.rect.x <= animation_end_point[0]:
        screen.blit(bg_img, (0, 0))
        # For each frame, update the position and image of James Bond
        james.update_animation()
        # We have also to update the coins in case there are some displayed in the screen
        for c in [s for s in all_sprites_list.spritedict if isinstance(s, Classes.Coin)]:
            c.update_animation()
        # It shows the current level, the text size is smaller each frame
...
def main():
    ...
    while True:
        ...
        Classes.Coin.add(total_level_width, total_level_height, obstacle_loc, all_sprites_list)  # add coins

        # Play the sounds
        bond_voice.play()
        main_theme.play(-1)

        # place james at correct position for the start animation
        ...
        while not james.dead:
            ...
            james.update(down, left, right, level)  # james moves according to key presses
            # Detect if James has collided with something
            james.collide(all_sprites_list)

            # Update the obstacles according to their types, agents have to track James and coins must revolve around
            # their axis
